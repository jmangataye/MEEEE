...
// MISE À JOUR 30/08/2026 — passe par la couche d'abstraction multi-modèle
// (voir lib/aiProviders/) au lieu d'appeler le SDK Anthropic directement.
// Comportement inchangé tant qu'OPENAI_API_KEY n'est pas configurée sur
// Render (Anthropic reste l'unique provider, mêmes erreurs qu'avant en cas
// de panne) — voir lib/aiProviders/index.js pour le détail du failover.
const { chatComplete } = require('./aiProviders');
const { recallRelevantMemories } = require('./embeddings');
    ...
    input_schema: {
      ...
      properties: {
        ...
        country: {
          type: 'string',
          description:
            'Rellena si el fan mencionó de qué país es, dónde vive, O una CIUDAD identificable — si conoces la ciudad, deduce el país (ej: menciona "Lima" o "Arequipa" → "Perú"; "Bogotá" o "Medellín" → "Colombia"; "CDMX", "Guadalajara" o "Monterrey" → "México"; "Buenos Aires" → "Argentina"; "Madrid" o "Barcelona" → "España"). Solo deduce el país a partir de una ciudad real que reconozcas con confianza — si la ciudad es ambigua entre varios países o no la reconoces, deja el campo vacío en vez de arriesgar una deducción incorrecta. NUNCA lo adivines a partir del idioma, el acento escrito, el uso de modismos, o cualquier otra suposición que no sea un país o una ciudad mencionados de forma concreta. Escribe el nombre del país en español, con mayúscula inicial (ej: "Colombia", no "colombia" ni "COL"). Deja vacío si no hay nada nuevo que anotar aquí. Se usa para el mapa de fans por país en el panel de la creadora.',
        },
      },
      required: ['notes'],
    },
...
function buildSystemPrompt({ settings, catalog, fan, purchasedItemIds = [], vaultSummary = [], relevantMemories = [] }) {
  ...
  // MISE À JOUR 30/08/2026 — mémoria vectorial (pgvector, ver lib/embeddings.js):
  // recuerdos antiguos que ya no caben en el resumen de "memory_notes" (o que
  // vienen de una nota anterior que fue reescrita), recuperados por similitud
  // semántica con el mensaje actual del fan. Vacío por defecto (no cambia el
  // comportamiento) mientras OPENAI_API_KEY no esté configurada en Render.
  const relevantMemoryText = relevantMemories.length
    ? `\nRECUERDOS ANTIGUOS RELEVANTES (recuperados automáticamente, pueden ser útiles para esta conversación puntual):\n${relevantMemories
        .map((m) => `- ${m.content}`)
        .join('\n')}`
    : '';
  ...
  // MISE À JOUR 02/09/2026 — nouveau champ libre "persona_identity" (dashboard,
  // Persona & Script → Identité), distinct de "tone" : "tone" décrit le
  // REGISTRE D'ÉCRITURE (comment ça sonne), "persona_identity" décrit le
  // CARACTÈRE/l'histoire du persona (qui il est). Optionnel — absent tant
  // qu'une créatrice n'a pas rempli ce champ, donc ne change rien par défaut.
  const identitySection = settings.persona_identity
    ? `\nIDENTIDAD Y PERSONALIDAD DE ${settings.persona_name}:\n${settings.persona_identity}\n`
    : '';
...
MEMORIA DEL FAN:
${memory}${profileSummary}${relevantMemoryText}
...
async function runAgentTurn({ settings, catalog, history, fanMessage, fan, purchasedItemIds = [], vaultSummary = [] }) {
  ...
  // Mémoire vectorielle (voir lib/embeddings.js) : no-op (tableau vide) tant
  // qu'OPENAI_API_KEY n'est pas configurée, ou si ce fan n'a encore aucun
  // souvenir stocké — donc aucun coût ajouté dans l'immense majorité des cas
  // actuels.
  const realFanId = fan && fan.id && fan.id !== 'test' && fan.id !== 'preview' ? fan.id : null;
  const relevantMemories = realFanId ? await recallRelevantMemories(realFanId, fanMessage) : [];

  const response = await chatComplete({
    max_tokens: 1024,
    system: buildSystemPrompt({ settings, catalog, fan, purchasedItemIds, vaultSummary, relevantMemories }),
    tools: buildTools(settings),
    messages,
  });

  // Journalise le coût réel de cet appel (voir dashboard, Vue d'ensemble →
  // panneau "Live Ops" → crédit IA estimé), et le provider qui a réellement
  // répondu (voir lib/aiProviders/ — 'anthropic' sauf bascule de secours).
  // fan_id n'est enregistré que pour un vrai fan (le simulateur "Tester le
  // bot" et l'aperçu du prompt utilisent des faux id "test"/"preview" qui ne
  // sont pas des uuid valides) — mais le coût lui-même est journalisé dans
  // tous les cas car il est bien réel.
  if (response.usage) {
    logAiUsage({
      ...
      // `settings.tenant_id` plutôt que fan.tenant_id : reste valable même
      // pour le simulateur "Tester le bot" / l'aperçu du prompt, qui utilisent
      // un faux fan sans tenant_id — le coût réel de cet appel doit quand même
      // être attribué au bon compte pour que l'estimation de crédit IA
      // (dashboard, Live Ops) reste juste par créatrice.
      tenant_id: settings.tenant_id,
    }).catch((err) => console.error('Erreur journalisation usage IA:', err.message));
  }

  return { text: response.text, toolCalls: response.toolCalls, raw: response.raw };
}
...
// MISE À JOUR 02/09/2026 — bouton "✨ Suggestion IA" PAR CHAMP (dashboard,
// Persona & Script → Identité), demandé par Bryan pour personnaliser le bot
// "de A à Z" (nom, identité/personnalité, ton) sans partir d'une page
// blanche. Volontairement distinct de generateScriptSuggestion() ci-dessus
// (qui remplit 6 champs de SCRIPT DE VENTE d'un coup) : ici on cible UN SEUL
// champ à la fois, pour que le dashboard puisse proposer la suggestion à côté
// du champ sans jamais écraser en silence ce que Bryan a déjà tapé ailleurs
// (voir generate-persona-suggestion dans server.js et suggestPersonaField()
// côté dashboard, qui n'applique qu'au clic sur "Utiliser").
const PERSONA_SUGGESTION_FIELDS = {
  persona_name: {
    label: 'nom du persona IA (assistante virtuelle)',
    instructions:
      'Propone ÚNICAMENTE un nombre de pila corto (1-2 palabras), cálido y creíble para una asistente virtual con IA de una creadora de contenido en Telegram. Sin apellido, sin emojis, sin comillas ni puntuación extra — solo el nombre.',
  },
  persona_identity: {
    label: 'identité / personnalité du persona IA (distincte du "tone", qui décrit le registre d\'écriture)',
    instructions:
      'Escribe una descripción de personalidad/identidad para esta asistente IA, en español, 2 a 4 frases: su carácter, un rasgo distintivo, o un pequeño detalle de "historia" que la haga sentir real — sin inventar datos personales sensibles ni contradecir el catálogo. Esto es una ficha interna para guiar a la IA, no un mensaje que se le envía al fan.',
  },
  tone: {
    label: 'ton (registre d\'écriture, ex: "cálida, cómplice, curiosa")',
    instructions:
      'Propone una descripción corta (una sola frase, 3 a 6 adjetivos o expresiones cortas separados por comas) del REGISTRO DE ESCRITURA de la IA — cómo suena al escribir, no su personalidad de fondo (eso ya está en "identidad").',
  },
};

async function generatePersonaSuggestion({ settings, field }) {
  const spec = PERSONA_SUGGESTION_FIELDS[field];
  if (!spec) throw new Error(`Champ de suggestion persona inconnu: ${field}`);

  const prompt = `Eres un experto en branding de asistentes virtuales de IA para creadoras de contenido en Telegram. Contexto actual (puede estar incompleto, es normal — no lo repitas, solo úsalo como referencia):
Nombre de la creadora: ${settings.creator_name || '(sin definir)'}
Nombre actual del persona IA: ${settings.persona_name || '(sin definir)'}
Identidad/personalidad actual: ${settings.persona_identity || '(sin definir)'}
Tono actual: ${settings.tone || '(sin definir)'}

Genera una propuesta SOLO para: ${spec.label}.
${spec.instructions}

Responde ÚNICAMENTE con un objeto JSON de una sola clave "suggestion" (string), sin texto antes ni después, sin bloque de código.`;
  ...
  if (response.usage) {
    logAiUsage({
      ...
      provider: response.provider,
      tenant_id: settings.tenant_id,
    }).catch((err) => console.error('Erreur journalisation usage IA (suggestion persona):', err.message));
  }
  ...
  if (typeof parsed.suggestion !== 'string' || !parsed.suggestion.trim()) {
    throw new Error('La IA no devolvió una suggestion exploitable.');
  }
  return parsed.suggestion.trim();
}

// ---------- Bilan business "Insights IA" (dashboard, Vue d'ensemble) ----------
// AJOUT 02/09/2026 — panneau "Insights IA" : un résumé court, en langage
// naturel, de la performance de la semaine, écrit par l'IA à partir de
// chiffres DÉJÀ calculés côté serveur (voir getBusinessInsightsStats() dans
// lib/supabase.js) — l'IA ne fait que les mettre en mots, elle ne les
// invente ni ne les recalcule, pour éviter tout risque d'hallucination de
// chiffre. Appel ponctuel, jamais automatique en arrière-plan : uniquement
// au clic explicite sur "Régénérer", ou si le cache a plus de 24h (voir la
// logique de cache dans /api/admin/generate-insights, server.js) — jamais à
// chaque chargement de page, pour limiter la consommation IA.
async function generateBusinessInsights({ settings, stats }) {
  const currency = settings.currency_symbol || '$';
  const fmtMoney = (n) => `${currency}${Math.round(Number(n) || 0)}`;
  const fmtPct = (n) => (n === null ? 'pas de comparaison possible (semaine précédente à zéro)' : `${n > 0 ? '+' : ''}${n}%`);

  const prompt = `Tu es un coach business pour créatrices de contenu, bienveillant mais direct (jamais mielleux, jamais corporate). ${settings.creator_name ? `La créatrice s'appelle ${settings.creator_name}.` : ''} Elle vend via un bot de vente automatisé sur Telegram. Voici les chiffres RÉELS de sa semaine, déjà calculés avec précision (ne les recalcule pas, ne les modifie pas, n'en invente aucun autre) :

Semaine en cours (7 derniers jours) :
- Nouveaux fans : ${stats.currentWeek.newFans}
- Ventes confirmées payées : ${stats.currentWeek.paidSalesCount} pour un total de ${fmtMoney(stats.currentWeek.paidRevenue)}
- Liens de paiement envoyés au total (payés ou non) : ${stats.currentWeek.linksSentCount}

Semaine précédente (7 jours d'avant), pour comparaison :
- Nouveaux fans : ${stats.previousWeek.newFans}
- Ventes confirmées payées : ${stats.previousWeek.paidSalesCount} pour un total de ${fmtMoney(stats.previousWeek.paidRevenue)}

Évolution semaine en cours vs semaine précédente :
- Nouveaux fans : ${fmtPct(stats.newFansChangePct)}
- Revenu confirmé : ${fmtPct(stats.revenueChangePct)}

Base de fans totale : ${stats.fanCountTotal} fans au total, dont ${stats.byStatus.customer || 0} ayant déjà acheté au moins une fois (taux de conversion global ${stats.conversionRatePct}%).

Rédige un bilan de 3 à 5 phrases MAXIMUM, en français, qui :
1. Résume la performance de la semaine avec les chiffres clés (fans, ventes, revenu) et leur évolution vs la semaine précédente.
2. Souligne UN point positif concret si les chiffres le permettent honnêtement.
3. Souligne UN point d'attention concret et actionnable si les chiffres le justifient (ex: baisse notable, taux de conversion faible, semaine sans vente) — sinon dis simplement que rien d'alarmant ne ressort, sans inventer un problème.
Écris un texte suivi et naturel, comme si tu parlais directement à la créatrice ("tu as..."), PAS une liste à puces mécanique des chiffres. N'invente AUCUN chiffre absent des données ci-dessus. Réponds UNIQUEMENT avec le texte du bilan — sans titre, sans guillemets, sans markdown, sans préambule.`;
  ...
  if (response.usage) {
    logAiUsage({
      ...
      provider: response.provider,
      tenant_id: settings.tenant_id,
    }).catch((err) => console.error('Erreur journalisation usage IA (insights business):', err.message));
  }

  const text = (response.text || '').trim();
  if (!text) throw new Error("La IA a retourné un texte vide pour le bilan.");
  return text;
}

module.exports = { runAgentTurn, buildSystemPrompt, generateScriptSuggestion, generatePersonaSuggestion, PERSONA_SUGGESTION_FIELDS, generateBusinessInsights };
