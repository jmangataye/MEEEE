...
const tools = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        agreed_price: { type: 'number', description: 'precio final acordado con el fan, en euros' },
        note_pour_le_fan: {
          type: 'string',
...
function buildSystemPrompt({ settings, catalog, fan }) {
  const catalogText = catalog
    .map(
      (it) =>
        `- id=${it.id} | ${it.name} (${it.tier}) — ${it.price}€${it.is_negotiable ? ' (negociable)' : ' (precio fijo)'} : ${it.description}`
    )
    .join('\n');
  ...
  const langInstruction =
    settings.language === 'es'
      ? 'Responde siempre en español, con un tono cálido y natural, como una colombiana cercana (uso mesurado de expresiones colombianas: "parce", "qué chévere", "uy", etc., sin exagerar ni caer en cliché).'
...
TU OBJETIVO:
Vender los contenidos del catálogo de abajo generando deseo, escuchando lo que busca el fan, y proponiendo la oferta más adecuada. Puedes negociar los artículos marcados como "negociable" pero nunca por debajo de ${settings.min_custom_price}€ ni con un descuento mayor al ${settings.max_negotiation_discount_pct}% del precio mostrado.
...
Responde siempre con mensajes cortos y naturales como en Telegram (nada de bloques de texto largos).`;
}

async function runAgentTurn({ settings, catalog, history, fanMessage, fan }) {
  const messages = [
    ...history.map((h) => ({
  ...
  const response = await anthropic.messages.create({
    model: MODEL,
    max_tokens: 1024,
    system: buildSystemPrompt({ settings, catalog, fan }),
    tools,
    messages,
  });
