...
async function getOrCreateFan({ telegram_user_id, telegram_username, first_name, source_token }) {
  ...
  try {
    assignedVariant = await pickVariantForField('intro_message');
  } catch (err) {
  ...
  const { data: created, error: createErr } = await supabase
    .from('fans')
    .insert({ telegram_user_id, telegram_username, first_name, source_token, ab_variant, variant_assignments })
    .select()
    .single();
  if (createErr) {
...
// l'assistant.
async function logMessage(fan_id, role, content, isFallback = false) {
  const { error } = await supabase.from('conversation_messages').insert({ fan_id, role, content, is_fallback: isFallback });
  if (error) throw error;
}
...
async function listFansWithPreview({ limit = 50, offset = 0, search = '', status = '' } = {}) {
  let query = supabase.from('fans').select('*');
...
async function logAiUsage({ input_tokens, output_tokens, fan_id, provider }) {
  const { error } = await supabase
    .from('ai_usage_log')
    .insert({
...
async function getAiUsageSince(sinceIso) {
  const { data, error } = await supabase.rpc('get_ai_usage_sum', { since_ts: sinceIso || null });
  if (error) throw error;
...
async function getVariantsForField(field_key, { activeOnly = true } = {}) {
  let query = supabase.from('script_variants').select('*').eq('field_key', field_key);
...
async function getVariantStats(field_key) {
  const variants = await getVariantsForField(field_key, { activeOnly: false });
...
async function pickVariantForField(field_key) {
  const stats = await getVariantStats(field_key);
...
async function logVariantEvent(variant_id, fan_id, event_type) {
  if (!variant_id) return;
  const { error } = await supabase.from('script_variant_events').insert({ variant_id, fan_id: fan_id || null, event_type });
  if (error) console.error('Erreur log événement variante:', error.message);
}

async function createScriptVariant({ field_key, label, content }) {
  const { data, error } = await supabase
    .from('script_variants')
    .insert({ field_key, label: label || null, content, active: true })
    .select()
    .single();
  if (error) throw error;
  return data;
...
async function maybeLogConversionForFan(fan_id) {
  try {
    ...
    await logVariantEvent(variantId, fan_id, 'converted');
  } catch (err) {
...
async function storeFanMemoryEmbedding(fan_id, content, embedding) {
  if (!embedding) return;
  const { error } = await supabase.from('fan_memory_embeddings').insert({ fan_id, content, embedding });
