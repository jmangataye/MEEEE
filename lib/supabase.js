...
async function listFansWithPreview({ tenant_id, limit = 50, offset = 0, search = '', status = '' } = {}) {
  ...
  const lastMessages = [];
  // MISE À JOUR 02/09/2026 — nombre total de messages échangés par fan (fan +
  // IA confondus), compté en même temps que l'aperçu du dernier message ci-
  // dessous (pas de requête supplémentaire : on compte simplement toutes les
  // lignes déjà rapatriées par ce même .in(fan_id, chunk)). Sert uniquement à
  // la détection optionnelle de sévérité "freeloader" côté dashboard (voir
  // freeloaderSeverity() dans public/admin/index.html) — désactivée par
  // défaut, purement indicative.
  const messageCountByFan = {};
  ...
  for (const m of lastMessages) {
    ...
    messageCountByFan[m.fan_id] = (messageCountByFan[m.fan_id] || 0) + 1;
  }

  // Nombre d'achats réellement PAYÉS par fan (statut "paid" uniquement — un
  // lien "sent" pas encore réglé ne compte pas comme un achat). Utilisé, en
  // plus de fans.total_spent, comme garde-fou de la détection freeloader :
  // un fan qui a déjà payé au moins une fois n'est jamais classé freeloader,
  // même s'il discute beaucoup depuis.
  const paidPurchaseCountByFan = {};
  for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
    const chunk = ids.slice(i, i + CHUNK_SIZE);
    ...
    (salesRows || []).forEach((s) => {
      paidPurchaseCountByFan[s.fan_id] = (paidPurchaseCountByFan[s.fan_id] || 0) + 1;
    });
  }

  return fans.map((f) => ({
    ...f,
    last_message: previewByFan[f.id] || null,
    message_count: messageCountByFan[f.id] || 0,
    paid_purchase_count: paidPurchaseCountByFan[f.id] || 0,
  }));
}
...
// ---------- Insights IA (bilan business, panneau "Insights IA" de la Vue
// d'ensemble) ----------
// AJOUT 02/09/2026 — mis en cache directement sur la ligne `settings` du
// tenant (colonnes ai_insights_text / ai_insights_generated_at), plutôt que
// dans une table dédiée : il n'y a qu'UN SEUL bilan actif à la fois par
// créatrice, pas d'historique à conserver ici (contrairement à
// settings_history). Écriture DIRECTE (pas via updateSettings()) pour ne PAS
// déclencher une photo dans settings_history à chaque régénération — ce
// n'est pas un réglage modifié par la créatrice, ça polluerait l'historique
// de Persona & Script avec des lignes non pertinentes.
//
// ⚠️ Nécessite que ces deux colonnes existent déjà sur la table `settings`
// dans Supabase — comme le reste du schéma de ce projet, aucune migration
// n'est versionnée dans ce repo (voir absence de dossier migrations/*.sql) ;
// à créer manuellement si ce n'est pas déjà fait :
//   ALTER TABLE settings ADD COLUMN IF NOT EXISTS ai_insights_text text;
//   ALTER TABLE settings ADD COLUMN IF NOT EXISTS ai_insights_generated_at timestamptz;
async function getBusinessInsightsCache(tenant_id) {
  if (!tenant_id) throw new Error('getBusinessInsightsCache() nécessite un tenant_id.');
  const { data, error } = await supabase
    .from('settings')
    .select('ai_insights_text, ai_insights_generated_at')
    .eq('tenant_id', tenant_id)
    .limit(1)
    .single();
  if (error) throw error;
...
async function saveBusinessInsightsCache(tenant_id, text) {
  if (!tenant_id) throw new Error('saveBusinessInsightsCache() nécessite un tenant_id.');
  const generated_at = new Date().toISOString();
  const { data, error } = await supabase
    .from('settings')
    .update({ ai_insights_text: text, ai_insights_generated_at: generated_at })
    .eq('tenant_id', tenant_id)
    .select('ai_insights_text, ai_insights_generated_at')
    .single();
  if (error) throw error;
...
// Agrège les chiffres nécessaires au bilan IA : semaine en cours vs semaine
// précédente (7 jours glissants chacune), plus quelques totaux globaux pour
// donner du contexte à l'IA. Volontairement calculé ici (pas laissé à l'IA)
// pour qu'elle ne puisse jamais halluciner un chiffre — elle ne fait que
// mettre en mots des nombres déjà exacts.
// Limite connue (partagée avec getAnalytics/getDailyTimeseries ci-dessus) :
// une vente est comptée sur sa date de CRÉATION du lien (`sales.created_at`),
// pas sur sa date de confirmation de paiement — la table `sales` n'a pas de
// colonne dédiée à ça (pas de webhook Dropp.fans, voir confirmSale()).
async function getBusinessInsightsStats(tenant_id) {
  if (!tenant_id) throw new Error('getBusinessInsightsStats() nécessite un tenant_id.');
  const day = 86400000;
  const since14Iso = new Date(Date.now() - 14 * day).toISOString();
  const [
    { data: recentSales, error: salesErr },
    { data: recentFans, error: fansErr },
    { count: fanCountTotal, error: fanCountErr },
    { data: allFanStatuses, error: statusErr },
  ] = await Promise.all([
    supabase.from('sales').select('price, status, created_at').eq('tenant_id', tenant_id).gte('created_at', since14Iso),
    supabase.from('fans').select('created_at').eq('tenant_id', tenant_id).gte('created_at', since14Iso),
    supabase.from('fans').select('*', { count: 'exact', head: true }).eq('tenant_id', tenant_id),
    supabase.from('fans').select('status').eq('tenant_id', tenant_id),
  ]);
  if (salesErr) throw salesErr;
  if (fansErr) throw fansErr;
  if (fanCountErr) throw fanCountErr;
  if (statusErr) throw statusErr;

  const now = Date.now();
  // startDaysAgo/endDaysAgo décrivent une fenêtre [now - startDaysAgo*day, now - endDaysAgo*day).
  const inWindow = (iso, startDaysAgo, endDaysAgo) => {
    const t = new Date(iso).getTime();
    return t >= now - startDaysAgo * day && t < now - endDaysAgo * day;
  };

  const currentWeekSales = (recentSales || []).filter((s) => inWindow(s.created_at, 7, 0));
  const previousWeekSales = (recentSales || []).filter((s) => inWindow(s.created_at, 14, 7));
  const currentWeekFans = (recentFans || []).filter((f) => inWindow(f.created_at, 7, 0));
  const previousWeekFans = (recentFans || []).filter((f) => inWindow(f.created_at, 14, 7));

  const sumPaid = (arr) => arr.filter((s) => s.status === 'paid').reduce((sum, s) => sum + Number(s.price), 0);
  const countPaid = (arr) => arr.filter((s) => s.status === 'paid').length;

  const currentWeek = {
    newFans: currentWeekFans.length,
    paidRevenue: sumPaid(currentWeekSales),
    paidSalesCount: countPaid(currentWeekSales),
    linksSentCount: currentWeekSales.length,
  };
  const previousWeek = {
    newFans: previousWeekFans.length,
    paidRevenue: sumPaid(previousWeekSales),
    paidSalesCount: countPaid(previousWeekSales),
    linksSentCount: previousWeekSales.length,
  };

  // null = "pas de base de comparaison" (semaine précédente à zéro) plutôt
  // qu'un pourcentage infini ou trompeur.
  ...
  const byStatus = {};
  (allFanStatuses || []).forEach((f) => {
    byStatus[f.status] = (byStatus[f.status] || 0) + 1;
  });
  const conversionRatePct = fanCountTotal
    ? Math.round(((byStatus.customer || 0) / fanCountTotal) * 1000) / 10
    : 0;

  return {
    currentWeek,
    previousWeek,
    fanCountTotal: fanCountTotal || 0,
    byStatus,
    conversionRatePct,
    newFansChangePct: pctChange(currentWeek.newFans, previousWeek.newFans),
    revenueChangePct: pctChange(currentWeek.paidRevenue, previousWeek.paidRevenue),
  };
}
...
// Prospects externes (créateurs/créatrices pas encore clients) — voir POST
// /api/waitlist dans server.js. Pas de tenant_id : ces personnes n'ont pas
// encore de compte Meeli. `waitlist_signups.email` porte une contrainte
// UNIQUE (voir migration create_waitlist_signups_table) : on distingue ici
// l'erreur "doublon" (code Postgres 23505) de toute autre erreur, pour que
// server.js puisse répondre gentiment plutôt que planter en 500.
async function addWaitlistSignup({ email, current_platform, follower_range, source }) {
  const { data, error } = await supabase
    .from('waitlist_signups')
    .insert({
      email,
      current_platform: current_platform || null,
      follower_range: follower_range || null,
      source: source || null,
    })
    .select()
    .single();
  ...
  return { alreadyRegistered: false, signup: data };
}
