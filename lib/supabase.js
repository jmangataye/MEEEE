...
async function getStalledConversations(thresholdMinutes = 5) {
  ...
  const { data: fans, error } = await supabase.from('fans').select('*').eq('paused', false);
  if (error) throw error;
  if (!fans || !fans.length) return [];

  const ids = fans.map((f) => f.id);
  const [{ data: lastMessages, error: msgErr }, { data: dismissals, error: dismissErr }] = await Promise.all([
    supabase
      .from('conversation_messages')
      .select('fan_id, role, content, created_at')
      .in('fan_id', ids)
      .order('created_at', { ascending: false }),
    supabase.from('stalled_dismissals').select('fan_id, dismissed_last_message_at').in('fan_id', ids),
  ]);
  ...
  const lastByFan = {};
  for (const m of lastMessages || []) {
    if (!lastByFan[m.fan_id]) lastByFan[m.fan_id] = m;
  }

  // Une conversation "dépilée" par l'admin ne réapparaît que si LE FAN a
  // écrit un nouveau message depuis — sinon elle resterait cachée pour
  // toujours alors que le blocage n'a pas changé. On compare l'horodatage du
  // dernier message au moment du dépilage à celui du dernier message actuel.
  const dismissedByFan = {};
  for (const d of dismissals || []) dismissedByFan[d.fan_id] = d.dismissed_last_message_at;

  const fanById = {};
  fans.forEach((f) => (fanById[f.id] = f));

  const stalled = [];
  for (const fanId of Object.keys(lastByFan)) {
    const m = lastByFan[fanId];
    if (m.role !== 'fan' || m.created_at >= cutoff) continue;
    const dismissedAt = dismissedByFan[fanId];
    if (dismissedAt && dismissedAt >= m.created_at) continue;
    const f = fanById[fanId];
    stalled.push({
      fan_id: fanId,
      ...
      minutes_since: Math.floor((Date.now() - new Date(m.created_at).getTime()) / 60000),
    });
  }
  // Un fan qui a déjà payé et reste bloqué sans réponse est bien plus urgent
  // qu'un fan gratuit tout neuf — priorité aux clients payants, puis au temps
  // d'attente (repéré le 30/08 : un client ayant payé $16.98 était resté 7h
  // sans réponse, noyé au milieu d'autres fans jamais convertis).
  return stalled.sort((a, b) => {
    const paidDiff = (Number(b.total_spent) > 0 ? 1 : 0) - (Number(a.total_spent) > 0 ? 1 : 0);
